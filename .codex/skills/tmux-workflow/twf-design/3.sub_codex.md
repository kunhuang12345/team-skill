...
- state file：`<state_dir>/<worker_full_name>.json`（默认；由 `scripts/twf_config.json` 的 `twf_state_dir_mode` 控制）
...
```jsonc
{
  ...
  "codex_start_cmd": "codex -c disable_paste_burst=true --sandbox danger-full-access -m <model> --config model_reasoning_effort=\"<effort>\"",
  "codex_resume_cmd": "codex -c disable_paste_burst=true --sandbox danger-full-access -m <model> --config model_reasoning_effort=\"<effort>\" resume 019b....", // 可选：缓存
