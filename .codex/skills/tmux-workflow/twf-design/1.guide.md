...
会话与状态文件：
- `twf` 的 state 目录由 `scripts/twf_config.json` 的 `twf_state_dir_mode` 控制（默认 `auto` → `<skill_root>/.twf/`；项目内默认：`./.codex/skills/tmux-workflow/.twf/`），也可用 `TWF_STATE_DIR` 覆盖；命名形如：`codex-a-YYYYmmdd-HHMMSS-<pid>.json`；使用 `codex-a` 时会自动选择最新的那个。
