...
ls -t ./.codex/skills/tmux-workflow/.twf/codex-a-*.json

# stop：只关 tmux，不删 state/codex_home（后续可 resume）
bash .codex/skills/tmux-workflow/scripts/twf stop codex-a

# resume：默认递归恢复子树（可加 --no-tree 只恢复自己）
bash .codex/skills/tmux-workflow/scripts/twf resume codex-a --no-tree

# spawn：创建子 worker 并维护 parent/children（parent 必须用全名）
bash .codex/skills/tmux-workflow/scripts/twf spawn codex-a-YYYYmmdd-HHMMSS-<pid> codex-sub

# remove：必须使用“全名”（带时间戳），默认递归删除子树（可加 --no-recursive 只删单点）
bash .codex/skills/tmux-workflow/scripts/twf remove codex-a-YYYYmmdd-HHMMSS-<pid>
bash .codex/skills/tmux-workflow/scripts/twf remove codex-a-YYYYmmdd-HHMMSS-<pid> --no-recursive
```

会话与状态文件：
- `twf` 的 state 目录由 `scripts/twf_config.yaml` 的 `twf_state_dir_mode` 控制（默认 `auto` → `<skill_root>/.twf/`；项目内默认：`./.codex/skills/tmux-workflow/.twf/`），也可用 `TWF_STATE_DIR` 覆盖；命名形如：`codex-a-YYYYmmdd-HHMMSS-<pid>.json`；使用 `codex-a` 时会自动选择最新的那个。
...
- 启动命令：
  - `tmux new-session -d -s "$SESSION" -c "$PWD" "env CODEX_HOME=<worker_home> codex -c disable_paste_burst=true --sandbox danger-full-access -m <model> --config model_reasoning_effort=\"<effort>\""`
- 写 session 文件（在 `$PWD`）：
  - `terminal=tmux`
...
- 对 Codex TUI：默认加 `-c disable_paste_burst=true --sandbox danger-full-access -m <model> --config model_reasoning_effort="<effort>"`（减少 paste burst 误判 + 避免沙箱权限反复询问 + 固定模型/effort）
