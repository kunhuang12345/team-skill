...
- 启动命令：
  - `tmux new-session -d -s "$SESSION" -c "$PWD" "env CODEX_HOME=<worker_home> codex -c disable_paste_burst=true --sandbox danger-full-access"`
- 写 session 文件（在 `$PWD`）：
  - `terminal=tmux`
...
- 对 Codex TUI：默认加 `-c disable_paste_burst=true --sandbox danger-full-access`（减少 paste burst 误判 + 避免沙箱权限反复询问）
