...
- `twf` 的 session/state 目录由 `scripts/twf_config.yaml` 的 `twf_state_dir_mode` 控制（默认 `auto` → `<skill_root>/.twf/`；项目内默认：`./.codex/skills/tmux-workflow/.twf/`），也可用 `TWF_STATE_DIR` 覆盖；文件为 `<full-name>.json`。
