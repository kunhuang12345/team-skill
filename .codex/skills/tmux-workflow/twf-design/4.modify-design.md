...
- No project-specific naming in this skill; everything is `twf` / `TWF_*`.
- Short, safe commands:
...
- `twf` state 目录由 `scripts/twf_config.yaml` 的 `twf_state_dir_mode` 控制：
...
- Default config file: `.codex/skills/tmux-workflow/scripts/twf_config.yaml`
...
- Start Codex with `-c disable_paste_burst=true --sandbox danger-full-access -m <model> --config model_reasoning_effort="<effort>"` (defaults from `scripts/twf_config.yaml`).
